...
def train_loop(device,model,loader,criterion,optimizer,scheduler):
    ...
    for i, data in enumerate(loader):
        ...
        output = model(bag, label)
        train_logits = output['logits']
        train_loss = output['loss']
        train_loss_log += output['loss'].item()
        train_loss.backward()
        optimizer.step()
...
def test_loop(
    ...
    """
    Test loop with the same interface as val_loop,
    but ALWAYS returns prediction outputs.
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            # ---------------- normal forward ----------------
            out = model(bag)
            val_logits = out["logits"].squeeze(0)   # [C]
            probs = torch.softmax(val_logits, dim=0)

            bag_predictions_after_normal.append(
                probs.cpu().numpy()
            )

            # ---------------- save outputs ----------------
            slide_paths.append(slide_path)
            logits_all.append(val_logits.cpu().numpy())
            probs_all.append(probs.cpu().numpy())

            # ---------------- optional loss ----------------
            if criterion is not None:
                val_logits_for_loss = val_logits.unsqueeze(0)
                if criterion.__class__.__name__ == "BCEWithLogitsLoss":
                    import torch.nn.functional as F
                    label_for_loss = F.one_hot(
                ...
                val_loss = criterion(
                    val_logits_for_loss, label_for_loss
                )
                test_loss_log += val_loss.item()
    ...
    test_loss_log = (
        ...
        if criterion is not None else 0.0
    )
    ...
    return (
        ...
        np.stack(logits_all, axis=0),
        np.stack(probs_all, axis=0),
    )
